...
def inputform(request):
    ...
    if (data.status_code) == 200:
        ...
        for i in mutualfund:
            name = i.split(';')
            code = i.split(';')[0]
            if(len(name) > 5 and code.isnumeric()):
                name = i.split(';')[-3]
...
def getmutualfund(request):
    try:
        ...
        try:
            ...
            print(previousYear)
            api = requests.get(
                "https://www.quandl.com/api/v3/datasets/AMFI/{}.json?api_key={}".format(plan, api_key))
            if api.status_code == 200:
                ...
                for i in apidata:
                    if(i[0] == previousYear):
                        ...
                        dictname.append({'Code': dataset['dataset_code'], 'Name': dataset['name'], 'Nav': nav, 'ReturnFund': str(round(ar, 2)), 'ReturnFundValue': str(
                            round(valueamount, 2)), 'InitialData': str(previousYear), 'Date': str(dataset['end_date']), 'amount': income, })
                        break
